...
@Component({
  selector: 'app-gradient-gauge',
  standalone: true,
  imports: [
    FormsModule
  ],
  templateUrl: './gradient-gauge.component.html',
  styleUrl: './gradient-gauge.component.scss'
})
export class GradientGaugeComponent implements OnInit
{
  ...
  private readonly blockSize: number = 2.5 * Math.PI / 180;
  private readonly gapSize: number = -0.0015 ;
  ...
  private createBlocksForValue ( value: number ): void
  {
    ...
    while ( currentAngle <= endAngle )
    {
      ...
      if ( b.endAngle >= endAngle )
      {
        // Mainly for the case where endAngle == startAngle :)
        b.opacity = 1.0 ;
      }
      else
      {
        const m : number = b.endAngle - startAngle ;
        const n : number = endAngle - startAngle ;
        const r : number = m / n ;
        b.opacity = 0.25 + 0.75 * r ;
      }
